  ...
  return <section className="creation-section section-pad paper-scene" id="technology"><AmbientAtmosphere paused={paused} tone="paper"/>
    ...
    <div className={`creation-panel creation-step-${step}`} id="creation-panel" role="tabpanel" aria-labelledby={`creation-tab-${step}`}>
      ...
      <div className="creation-media" key={step}>
        ...
        {step===1&&<figure className="character-sheet"><img src={`${import.meta.env.BASE_URL}assets/creation/character-views.webp`} width="1536" height="1024" alt={lang==='zh'?'银发男主正面、侧面和背面设定图':'Silver-haired character turnaround'} loading="lazy" fetchPriority="high" decoding="async"/><figcaption>CHARACTER STUDY / 001</figcaption></figure>}
        {step===2&&<div className="creation-model"><Suspense fallback={<span className="model-loading">Loading…</span>}><MascotModel source={`${import.meta.env.BASE_URL}assets/taisui-optimized.glb`} name={lang==='zh'?'范太岁':'Fan Taisui'} paused={paused} lang={lang}/></Suspense><span className="creation-model-hint">{lang==='zh'?'拖拽旋转 · 滚轮缩放':'DRAG TO ROTATE · SCROLL TO ZOOM'}</span></div>}
