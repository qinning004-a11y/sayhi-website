    ...
    <main>
      ...
      <section id="products" className="products-section section-pad" style={{ '--product-color': product.color }}>
        ...
        <div id="product-panel" role="tabpanel" aria-labelledby={`product-tab-${activeProduct}`} className="product-showcase">
          <div className="product-visual"><img key={product.image} src={product.image} loading="lazy" fetchPriority="high" decoding="async" alt={text.scene}/><span className="image-label">{c.concept}</span><div className="visual-caption"><span>{product.word}</span><span>{text.scene}</span></div><button className="preview-circle" onClick={() => setPreview(product)} aria-label={`${c.preview} — ${product.name}`}><ArrowUpRight size={32}/></button></div>
